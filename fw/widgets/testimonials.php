...
/* Load the widget */
add_action( 'widgets_init', array( 'Azzu_Widgets_Testimonials', 'azzu_register_widget' ) );

class Azzu_Widgets_Testimonials extends WP_Widget {
	...
	function __construct() {  
		/* Widget settings. */
		$widget_ops = array( 'description' => _x( 'Testimonials', 'widget', 'azzu'.LANG_DN ) );

		/* Create the widget. */
		parent::__construct(
			'azzu-testimonials',
			AZU_WIDGET_PREFIX . _x( 'Testimonials', 'widget', 'azzu'.LANG_DN ),
			$widget_ops
		);
	}
	...
	function widget( $args, $instance ) {
		...
		if ( $title ) {
			echo $before_title . $title . $after_title . "\n";
                ...
                echo azut()->azzu_call_shortcode('azu_testimonials',$attr);
	...
	function form( $instance ) {
		...
		$terms = get_terms( 'azu_testimonials_category', array(
			'hide_empty'    => 1,
			'hierarchical'  => false 
		) );
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'show' ); ?>"><?php _ex('Number of testimonials:', 'widget', 'azzu'.LANG_DN); ?></label>
