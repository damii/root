...
//theme added
$azu_blur = in_array($azu_blur, array('row', 'col') ) ? $azu_blur : '';
if($azu_blur == 'row')
    $el_class .= ' azu-blurred-row';
else if($azu_blur == 'col')
    $el_class .= ' azu-blurred-col';
...
if ( $anchor ) {
        ...
        $container_data_attr[] = 'id="' . esc_attr( $anchor.'-anchor' ) . '"';
}

if ( '' !== $min_height ) {
        $container_data_attr[] = 'data-min-height="' . esc_attr( $min_height ) . '"';
}
$wrapper_attributes[] = implode(' ', $container_data_attr);
