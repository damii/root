 ...
 *
 * @since azzu 1.0
 * Class renamed to AzuBreadcrumb
 */
class AzuBreadcrumb {
	...
	public function __construct( $args = array() ) {
		...
		$defaults = array(
			'container'       => 'div',
			'separator'       => '<i class="azu-icon-sep"></i>',
			'before'          => '',
			'after'           => '',
			'show_on_front'   => true,
			'network'         => false,
			...
			/* Labels for text used (see AzuBreadcrumb::default_labels). */
			'labels' => array()
		);
	...
	public function trail() {
		...
		if ( !empty( $this->items ) && is_array( $this->items ) ) {
			...
			array_push( $this->items, '<span class="trail-end active">' . array_pop( $this->items ) . '</span>' );
	...
	public function default_labels() {
		...
		$labels = array(
			'browse'              => _x( 'Browse:',                             'breadcrumb', 'azzu'.LANG_DN),
			'home'                => _x( 'Home',                                'breadcrumb', 'azzu'.LANG_DN),
			'error_404'           => _x( '404 Not Found',                       'breadcrumb', 'azzu'.LANG_DN),
			'archives'            => _x( 'Archives',                            'breadcrumb', 'azzu'.LANG_DN),
			/* Translators: %s is the search query. The HTML entities are opening and closing curly quotes. */
			'search'              => _x( 'Search results for &#8220;%s&#8221;', 'breadcrumb', 'azzu'.LANG_DN),
			/* Translators: %s is the page number. */
			'paged'               => _x( 'Page %s',                             'breadcrumb', 'azzu'.LANG_DN),
			/* Translators: Minute archive title. %s is the minute time format. */
			'archive_minute'      => _x( 'Minute %s',                           'breadcrumb', 'azzu'.LANG_DN),
			/* Translators: Weekly archive title. %s is the week date format. */
			'archive_week'        => _x( 'Week %s',                             'breadcrumb', 'azzu'.LANG_DN),
	...
	public function do_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->args['labels']['archive_minute_hour'], get_the_time( esc_html(_x( 'g:i a', 'minute and hour archives time format', 'azzu'.LANG_DN) )) );
	}
	...
	public function do_minute_archive_items() {
		...
		/* Add the minute item. */
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->args['labels']['archive_minute'], get_the_time( esc_html(_x( 'i', 'minute archives time format', 'azzu'.LANG_DN) )) );
	}
	...
	public function do_hour_archive_items() {
		...
		/* Add the hour item. */
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->args['labels']['archive_hour'], get_the_time( esc_html(_x( 'g a', 'hour archives time format', 'azzu'.LANG_DN) )) );
	}
	...
	public function do_day_archive_items() {
		...
		$year  = sprintf( $this->args['labels']['archive_year'],  get_the_time( esc_html( _x( 'Y', 'yearly archives date format',  'azzu'.LANG_DN) )) );
		$month = sprintf( $this->args['labels']['archive_month'], get_the_time( esc_html(_x( 'F', 'monthly archives date format', 'azzu'.LANG_DN) )) );
		$day   = sprintf( $this->args['labels']['archive_day'],   get_the_time( esc_html(_x( 'j', 'daily archives date format', 'azzu'.LANG_DN) )) );
	...
	public function do_week_archive_items() {
		...
		$year = sprintf( $this->args['labels']['archive_year'],  get_the_time( esc_html(_x( 'Y', 'yearly archives date format', 'azzu'.LANG_DN) )) );
		$week = sprintf( $this->args['labels']['archive_week'],  get_the_time( esc_html(_x( 'W', 'weekly archives date format', 'azzu'.LANG_DN) )) );
		...
		/* Add the week item. */
		if ( is_paged() )
			$this->items[] = get_archives_link( esc_url_raw( add_query_arg( array( 'm' => get_the_time( 'Y' ), 'w' => get_the_time( 'W' ) ), home_url() ) ) , $week, false );
	...
	public function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = '<a href="' . get_year_link( get_the_time( 'Y', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_year'], get_the_time( esc_html(_x( 'Y', 'yearly archives date format', 'azzu'.LANG_DN) )) ) . '</a>';

				/* If using the %monthnum% tag, add a link to the monthly archive. */
				elseif ( '%monthnum%' == $tag )
					$this->items[] = '<a href="' . get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_month'], get_the_time( esc_html(_x( 'F', 'monthly archives date format', 'azzu'.LANG_DN) )) ) . '</a>';

				/* If using the %day% tag, add a link to the daily archive. */
				elseif ( '%day%' == $tag )
					$this->items[] = '<a href="' . get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_day'], get_the_time( esc_html(_x( 'j', 'daily archives date format', 'azzu'.LANG_DN) )) ) . '</a>';
...
/**
 * Extends the AzuBreadcrumb class for bbPress.  Only use this if bbPress is in use.  This should 
 * serve as an example for other plugin developers to build custom breadcrumb items.
 *
 * @since azzu 1.0
 * @access public
 */
class bbPress_AzuBreadcrumb extends AzuBreadcrumb {
	...
	public function do_trail_items() {
		...
		elseif ( bbp_is_topic_tag_edit() ) {
			...
			if ( true === $this->args['show_title'] )
				$this->items[] = _x( 'Edit', 'breadcrumb', 'azzu'.LANG_DN);
		}
		...
		elseif ( bbp_is_single_topic() ) {
			...
			if ( bbp_is_topic_split() && true === $this->args['show_title'] )
				$this->items[] = _x( 'Split', 'breadcrumb', 'azzu'.LANG_DN);

			/* If viewing a topic merge page. */
			elseif ( bbp_is_topic_merge() && true === $this->args['show_title'] )
				$this->items[] = _x( 'Merge', 'breadcrumb', 'azzu'.LANG_DN);
...
/**
 * Extends the AzuBreadcrumb class for wooCommerce.  Only use this if wooCommerce is in use.  This should 
 * serve as an example for other plugin developers to build custom breadcrumb items.
 *
 * @since azzu 1.0
 * @access public
 */
class woo_AzuBreadcrumb {
	/**
	 * Arguments used to build the breadcrumb trail.
	 *
	 * @since azzu 1.0
	...
	public function __construct( $args = array() ) {
		...
		$defaults = array(
			'delimiter'   => '<i class="azu-icon-sep"></i>',
		);
	...
	public function trail() {
		if ( true === $this->args['echo'] )
			echo woocommerce_breadcrumb( $this->args );
		else {
                    ob_start();
                    woocommerce_breadcrumb( $this->args );
