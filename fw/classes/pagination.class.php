...
class WPBootstrap_Pager {
	...
	public static function archive_pager( $args = array() ) {
        ...
        /**
	 * Provide pager interface for custom post-type pages with ajax
	 * 
	 * @param array $args See README for a list of arguments.
	 * @return string Pager output
	 */
	public static function posttype_pager( $args = array(), $current_page = 1 , $max_num_pages = 1 ) {
		$args = self::_parse_args( $args );
		
		return self::_pager_output( $current_page, $max_num_pages, $args, 'archive' );
	}
	...
	protected static function _parse_args( $args ) {
		$defaults = array(
			...
			'current_after'    => '</li>',
			'currentlink'      => '% <span class="sr-only">'. __('(current)','azzu'.LANG_DN) . '</span>',
	...
	protected static function _pager_output( $current_page, $num_pages, $args, $type ) {
		...
		if ( 1 < $num_pages ) {
			...
			if ( 'number' == $next_or_number ) {
				...
				$output .= $b . self::_get_page_link($current_page - 1, $type, 'azu-no-border') . $previouspagelink . '</a>' . $a;
				...
				$output .= $b . self::_get_page_link($current_page + 1, $type, 'azu-no-border') . $nextpagelink . '</a>' . $a;
                ...
                else if($type=='post' && 'next' == $next_or_number){ // if ( 'number' == $next_or_number )
				//single page only
                                // Display next/previous links instead of page numbers
				...
				$output .= $before;
                                ...
                                {
                                    $previous_post_link = '<a style="visibility: hidden;" href="#"></a>';
                                }
                                $output .= $previous_before . self::_get_link_formater($previous_post_link , '<i class="azu-icon-left-pagi"></i>'.$previouspagelink) . $previous_after;

                                
                                $output .= '<li class="azu-pagi-center"><a href="'. home_url( '/?post_type='.get_post_type() ) .'" rel="center"><i class="azu-icon-center-pagi"></i></a></li>';
					
				// Link to next page
                                if ( !$next_post_link )
                                {
                                    $next_post_link = '<a style="visibility: hidden;" href="#"></a>';
                                }
                                $output .= $next_before . self::_get_link_formater($next_post_link, $nextpagelink.'<i class="azu-icon-right-pagi"></i>') . $next_after;
        ...
        protected static function _get_link_formater( $html, $text ) {
            //<a href="%link" rel="prev|Next">%title</a>
            $html = str_replace('</a>','',$html);
            $html = str_replace('>','> <span title="',$html);
            $html .= '">'.$text.'</span></a>';
            return $html;
        }
	 ...
	 */
	protected static function _get_page_link( $pagenum, $type, $class='' ) {
		if ( 'post' == $type ) {
			return _wp_link_page( $pagenum );
		} else if ( 'archive' == $type ) {
			return '<a href="' . get_pagenum_link( $pagenum ) . '" '.( !empty($class) ? 'class="'.$class.'"' : '').' data-page-num="'.$pagenum.'" >';
		}
	}
}
