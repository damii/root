        ...
        */
				...
				<div id="redux-add-widget" class="widgets-holder-wrap">
				 <div class="">
				  ...
				  <div class="sidebar-name">
				   <h3><?php echo __('Create Widget Area', 'azzu'.LANG_DN); ?> <span class="spinner"></span></h3>
				  </div>
				  <div class="sidebar-description">
					<form id="addWidgetAreaForm" action="" method="post">
				      <div class="widget-content">
				        <input id="redux-add-widget-input" name="redux-add-widget-input" type="text" class="regular-text" title="<?php echo __('Name', 'azzu'.LANG_DN); ?>" placeholder="<?php echo __('Name', 'azzu'.LANG_DN); ?>" />
				      </div>
				      <div class="widget-control-actions">
				        <div class="aligncenter">
				          <input class="addWidgetArea-button button-primary" type="submit" value="<?php echo __('Create Widget Area', 'azzu'.LANG_DN); ?>" />
				        </div>
				        <br class="clear">
				      </div>
				    </form>
        ...
        function check_widget_area_name($name) {
            ...
            if(!empty($wp_registered_sidebars)) 
                $reg_widget_areas = $wp_registered_sidebars;
            else if(!empty($GLOBALS['wp_registered_widget_areas'])) 
                $reg_widget_areas = $GLOBALS['wp_registered_widget_areas'];
            else
                return $name;

            $taken = array();
            foreach ( $reg_widget_areas as $widget_area ) {
                $taken[] = strtolower($widget_area['name']);
            }

            $taken = array_merge($taken, $this->widget_areas);

            if(in_array(strtolower($name), $taken)) {
        ...
        function save_widget_areas() {
        	set_theme_mod( 'azu-widget-areas', array_unique( $this->widget_areas ));
        }
        ...
        function register_custom_widget_areas() {
            ...
            $options = array(
              ...
              'before_widget' => '<div id="%1$s" class="azu-widget clearfix %2$s">', 
              'after_widget'  => '</div>'
            );
            ...
            if(is_array($this->widget_areas)) {
                foreach (array_unique($this->widget_areas) as $widget_area) { 
                    ...
                    $options['description'] = _x( 'Widget area created by user', 'atheme', 'azzu'.LANG_DN );
        ...
        public function get_widget_areas() {
            ...
            $db = get_theme_mod('azu-widget-areas');
