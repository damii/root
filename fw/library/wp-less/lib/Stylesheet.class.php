...
class WPLessStylesheet
{
  ...
  protected function configurePath()
  {
    ...
    if ( defined( 'SUNRISE' ) && SUNRISE =='on' ){
        $content_less_path = explode('wp-content', $this->stylesheet->src, 2);
        $content_less_path = count($content_less_path) > 1 ? $content_less_path[1] :$content_less_path[0]; 
    }
    else
        $content_less_path = preg_replace('#^'.content_url().'#U', '', $this->stylesheet->src);
    $this->source_path =    WP_CONTENT_DIR.$content_less_path;
  ...
  {
    $this->signature = apply_filters( 'wp-less_configure_signature', substr(sha1(serialize($variables) . $this->source_timestamp), 0, 10) );
  }
  ...
  {
    return apply_filters('wp-less_stylesheet_source_content', azu_file_get_c($this->source_path));
  }
