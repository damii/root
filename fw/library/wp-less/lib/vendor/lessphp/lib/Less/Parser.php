...
class Less_Parser{
	...
	private function GetRules( $file_path ){
		...
		if( $cache_file ){
			if( Less_Parser::$options['cache_method'] == 'callback' ){
				...
				switch(Less_Parser::$options['cache_method']){
					...
					case 'serialize':
						$cache = unserialize(azu_file_get_c($cache_file));
						if( $cache ){
							touch($cache_file);
							$this->UnsetInput();
							return $cache;
		...
		if( $cache_file ){
			if( Less_Parser::$options['cache_method'] == 'callback' ){
				...
				switch(Less_Parser::$options['cache_method']){
					case 'serialize':
						azu_file_put_c( $cache_file, serialize($rules) );
					break;
					case 'php':
						azu_file_put_c( $cache_file, '<?php return '.self::ArgString($rules).'; ?>' );
					break;
					case 'var_export':
						//Requires __set_state()
						azu_file_put_c( $cache_file, '<?php return '.var_export($rules,true).'; ?>' );
					break;
				}
	...
	public function SetInput( $file_path ){
		...
		if( $file_path ){
			$this->input = azu_file_get_c( $file_path );
		}
