	...
	class RWMB_File_Field extends RWMB_Field
	{
		...
		static function admin_enqueue_scripts()
		{
			...
			wp_localize_script( 'rwmb-file', 'rwmbFile', array(
				'maxFileUploadsSingle' => __( 'You may only upload maximum %d file', 'azzu'.LANG_DN ),
				'maxFileUploadsPlural' => __( 'You may only upload maximum %d files', 'azzu'.LANG_DN ),
			) );
		}
		...
		static function wp_ajax_delete_file()
		{
			...
			if ( $ok )
				...
				wp_send_json_error( __( 'Error: Cannot delete file', 'azzu'.LANG_DN ) );
		}
		...
		{
			$i18n_title = apply_filters( 'rwmb_file_upload_string', _x( 'Upload Files', 'file upload', 'azzu'.LANG_DN ), $field );
			$i18n_more  = apply_filters( 'rwmb_file_add_string', _x( '+ Add new file', 'file upload', 'azzu'.LANG_DN ), $field );
		...
		{
			$i18n_delete = apply_filters( 'rwmb_file_delete_string', _x( 'Delete', 'file upload', 'azzu'.LANG_DN ) );
			$i18n_edit   = apply_filters( 'rwmb_file_edit_string', _x( 'Edit', 'file upload', 'azzu'.LANG_DN ) );
			$li          = '
			<li id="item_%s">
